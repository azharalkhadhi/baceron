...
class Auth extends CI_Controller
{
    ...
    private function _login()
    {
        ...
        if ($user) {
            ...
            if ($user['password'] == md5($password)) {
                ...
                $this->session->set_flashdata('info', '<h4 class="danger">password salah!</h5>');
                redirect('auth/signin');
            }
        } else {
            $this->session->set_flashdata('info', '<h4 class="danger">username tidak ditermukan!</h5>');
            redirect('auth/signin');
        }
    }
    ...
    public function signUp()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('info', '<h4 class="success">Registrasi berhasil! silahkan login</h5>');
            redirect('auth/signin');
        }
    }
